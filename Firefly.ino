...
void setup () {
	...
	wdText = HT1632.getTextWidth(messages[msgindex], FONT_8X4_END, FONT_8X4_HEIGHT);
...
void loop () {
	...
	switch(displayMode) {
		case displayGameBlock:
			doModeGameBlock();
			break;
		case displayGameSnake:
			doModegameSnake();
			break;
		case displaySparkle:
			doModeSparkle();
...
void getNextMode() {
	...
	if ( displayMode == displayImage ) {
		...
		displayMode = displayThanksDom;
		msgindex=0;
	} else if ( displayMode == displayThanksDom ) {
		displayMode = displayThanksDesgn;
		msgindex=1;
	} else if ( displayMode == displayThanksDesgn ) {
...
void doModeText() {
	HT1632.clear();
	HT1632.drawTextFF(messages[msgindex], 16 - i, 2, FONT_8X4, FONT_8X4_END, FONT_8X4_HEIGHT);
}
